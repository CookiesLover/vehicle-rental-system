...
public class WebController {
    ...
    public String adminPage(HttpSession session,
        ...
        model.addAttribute("vehicles", rentalService.getVehicles());
        model.addAttribute("customers", rentalService.getCustomers());

        return "admin";
    }

    @PostMapping("/admin/vehicle/add")
    public String adminAddVehicle(@RequestParam int id,
        ...
        boolean ok = rentalService.addVehicle(new Vehicle(id, brand, modelName, type, pricePerDay, imageUrl));
        if (!ok) return "redirect:/admin?msg=ОШИБКА:%20ID%20уже%20существует";

        return "redirect:/admin?msg=ОК:%20Авто%20добавлено";
    }

    @PostMapping("/admin/vehicle/update")
    public String adminUpdateVehicle(@RequestParam int id,
                                     @RequestParam String brand,
                                     @RequestParam String modelName,
                                     @RequestParam String type,
                                     @RequestParam double pricePerDay,
        ...
        String result = rentalService.updateVehicle(id, brand, modelName, type, pricePerDay, imageUrl);
        return "redirect:/admin?msg=" + urlEncode(result);
    }

    @PostMapping("/admin/vehicle/delete")
    public String adminDeleteVehicle(@RequestParam int id, HttpSession session) {
        if (!isAdmin(session)) return "redirect:/vehicles?msg=Нет%20доступа";

        String result = rentalService.deleteVehicle(id);
        return "redirect:/admin?msg=" + urlEncode(result);
    }

    @PostMapping("/admin/customer/add")
    public String adminAddCustomer(@RequestParam int id,
                                   @RequestParam String fullName,
                                   @RequestParam String phone,
                                   HttpSession session) {
        if (!isAdmin(session)) return "redirect:/vehicles?msg=Нет%20доступа";

        boolean ok = rentalService.addCustomer(new Customer(id, fullName, phone));
        if (!ok) return "redirect:/admin?msg=ОШИБКА:%20ID%20клиента%20уже%20существует";
